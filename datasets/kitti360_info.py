...
name2label = {label.name: label for label in labels_info}
label2name = {label.id: label.name for label in labels_info}
# id to label objectj
id2label = {label.id: label for label in labels_info}
# trainId to label object
trainId2label = {label.trainId: label for label in reversed(labels_info)}
# KITTI-360 ID to cityscapes ID
kittiId2label = {label.kittiId: label for label in labels_info}
# IDs to colors
...
# Step 1: Create a dictionary that maps train_id to whether it's a "thing" or not
train_id_to_is_thing = {}

for label in labels_info:
    # Only add the train_id if it hasn't been added yet (i.e., the first occurrence)
    if label.trainId not in train_id_to_is_thing:
        train_id_to_is_thing[label.trainId] = label.hasInstances


# Step 2: Define a function to check if a given train_id is a thing or not
def is_thing_by_train_id(train_id):
    return train_id_to_is_thing.get(train_id, False)  # Return False if train_id not found


# Example usage: Check if the updated train_ids are "things"
updated_train_ids = np.array([0, 1, 2, 3, 255])  # Example updated_train_ids array
is_thing_labels = np.vectorize(is_thing_by_train_id)(updated_train_ids)

print(is_thing_labels)  # Output: array of True/False indicating if each train_id is a thing
...
# Define the is_thing function
def is_thing_kitti360(label_id):
    id_to_hasInstances_map = {label.id: label.hasInstances for label in labels_info}
    return id_to_hasInstances_map.get(label_id, False)  # Returns True if it's a thing, False otherwise


# Define a function to map the semantic labels using the dictionary
def map_labels_kitti360(label):
    mapping = {label.id: label.trainId for label in labels_info}
    return mapping.get(label, label)  # Leave the label unchanged if it's not in the map


def get_things_stuff_split_kitti360():
    # Initialize sets for things and stuff
    thing_labels = set()
    stuff_labels = set()
...
# Now, thing_labels contains the names of labels that have instances (things)
# and stuff_labels contains the names of labels that don't have instances (stuff)

print("Thing labels:", thing_labels)
print("Stuff labels:", stuff_labels)
