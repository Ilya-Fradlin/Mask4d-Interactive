...
class UserInteractiveSegmentationModel(abc.ABC):
    def __init__(self, device, config, dataloader_test):
        ...
        self.model = Interactive4D(num_heads=8, num_decoders=3, num_levels=1, hidden_dim=128, dim_feedforward=1024, shared_decoder=False, num_bg_queries=10, dropout=0.0, pre_norm=False, aux=True, voxel_size=config.data.voxel_size, sample_sizes=[4000, 8000, 16000, 32000], use_objid_attention=config.general.use_objid_attention)
        self.model.to(device)
        self.model.eval()

        self.device = device
        if self.pretrained_weights_file:
            # self.load_weights(self.pretrained_weights_file)
            lightning_model = ObjectSegmentation.load_from_checkpoint(self.pretrained_weights_file)
        ...
        self.cube_size = self.config.cubeedge if self.config.cubeedge else 0.1  # the size of the segmentation mask for a single selected point
