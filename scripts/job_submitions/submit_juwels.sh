...
sbatch --account=objectsegvideo --nodes=4 --ntasks-per-node=4 --gres=gpu:4 --output=outputs/%j_Interactive4d.txt --time=00-23:59:59 --partition=booster --signal=TERM@120 --mail-user=ilya.fradlin@rwth-aachen.de --mail-type=BEGIN,END,FAIL scripts/job_submitions/run_on_node.sh
